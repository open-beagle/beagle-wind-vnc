...
# 安装GStreamer 1.28.1 运行时依赖包
# 彻底清理了 1.22 时代的遗留依赖，移除了庞大的 -dev 开发包和 x264/x265 CLI 工具。
# 这些包是通过 ldd 直接比对 bdwind-gstreamer 1.28.1 tarball 内所有的 .so 精准备齐的运行库。
...
# 私有化定制：使用刚编译好的 1.28.1 专用底座与 Python 包合集
cd /tmp
echo "Downloading custom bdwind-gstreamer 1.28.1 tarball..."
curl -O -fsSL "https://cache.ali.wodcloud.com/vscode/bdwind/bdwind-gstreamer-1.28.1-ubuntu24.04.tar.gz"
echo "Extracting GStreamer Custom Engines into /opt..."
tar -xzf bdwind-gstreamer-1.28.1-ubuntu24.04.tar.gz -C /opt
rm -f bdwind-gstreamer-1.28.1-ubuntu24.04.tar.gz
...
# 清理解压包、系统缓存和临时文件
rm -f /tmp/bdwind-gstreamer-1.28.1-ubuntu24.04.tar.gz
