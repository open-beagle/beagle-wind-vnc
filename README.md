...
| 参数                   | 说明                                 | 默认值      |
| :--------------------- | :----------------------------------- | :---------- |
| `BDWIND_ENCODER`       | 视频编码器（详见下方编码器优劣对比） | `x264enc` |
...
- **`x264enc` (H.264 - 官方默认 CPU 软编)**
  - 适用：无独立显卡的云主机、或者需要将 GPU 资源 100% 留给推理计算的场景。
  - 特点：兼容性最高，开箱即用，画质细腻，但延迟略高于硬件编码。

- **`nvh264enc` / `nvcudah264enc` (H.264 - 推荐 GPU 硬编)**
