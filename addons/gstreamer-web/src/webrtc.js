...
class WebRTCDemo {
  ...
  constructor(signalling, element, peer_id) {
    ...
    this.input = new Input(this.element, (data) => {
      if (
        this._connected &&
        this._send_channel !== null &&
        this._send_channel.readyState === "open"
      ) {
  ...
  _onSignallingICE(icecandidate) {
    ...
    if (this.forceTurn && JSON.stringify(icecandidate).indexOf("relay") < 0) {
      ...
      this._setDebug(
        "Rejecting non-relay ICE candidate: " + JSON.stringify(icecandidate)
      );
      return;
    }
    ...
    this.peerConnection.setRemoteDescription(sdp).then(() => {
      ...
      this.peerConnection
        ...
        .then((local_sdp) => {
          ...
          if (
            !/[^-]sps-pps-idr-in-keyframe=1[^\d]/gm.test(local_sdp.sdp) &&
            /[^-]packetization-mode=/gm.test(local_sdp.sdp)
          ) {
            console.log(
              "Overriding WebRTC SDP to include sps-pps-idr-in-keyframe=1"
            );
            if (/[^-]sps-pps-idr-in-keyframe=\d+/gm.test(local_sdp.sdp)) {
              local_sdp.sdp = local_sdp.sdp.replace(
          ...
          if (local_sdp.sdp.indexOf("multiopus") === -1) {
            ...
            if (
              !/[^-]stereo=1[^\d]/gm.test(local_sdp.sdp) &&
              /[^-]useinbandfec=/gm.test(local_sdp.sdp)
            ) {
            ...
            if (
              !/[^-]minptime=10[^\d]/gm.test(local_sdp.sdp) &&
              /[^-]useinbandfec=/gm.test(local_sdp.sdp)
            ) {
              console.log(
                "Overriding WebRTC SDP to allow low-latency audio packet"
              );
  ...
  _ontrack(event) {
    this._setStatus(
      "Received incoming " + event.track.kind + " stream from peer"
    );
  ...
  _onPeerDataChannelMessage(event) {
    ...
    try {
      ...
      if (e instanceof SyntaxError) {
        this._setError(
          "error parsing data channel message as JSON: " + event.data
        );
      } else {
    ...
    if (msg.type === "pipeline") {
      ...
      if (msg.data !== null) {
        ...
        this._setDebug(
          "received clipboard contents, length: " + content.length
        );
      ...
      if (this.oncursorchange !== null && msg.data !== null) {
        ...
        this._setDebug(
          `received new cursor contents, handle: ${handle}, hotspot: ${JSON.stringify(
            hotspot
          )} image length: ${curdata.length}`
        );
  ...
  sendDataChannelMessage(message) {
    if (
      ...
      this._setError(
        "attempt to send data channel message before channel was open."
      );
    }
  }
      ...
      pc.getStats()
        .then((stats) => {
          ...
          if (videoRTP !== null) {
            ...
            connectionDetails.video.decoder =
              videoRTP.decoderImplementation || "unknown";
            ...
            if (codec !== undefined) {
              connectionDetails.video.codecName = codec.mimeType
                .split("/")[1]
                .toUpperCase();
            }
          }
          ...
          if (audioRTP !== null) {
            ...
            if (codec !== undefined) {
              connectionDetails.audio.codecName = codec.mimeType
                .split("/")[1]
                .toUpperCase();
            }
          }
          ...
          if (dataChannel !== null) {
            ...
            connectionDetails.data.messagesReceived =
              dataChannel.messagesReceived;
          ...
          if (Object.keys(reports.transports).length > 0) {
            var transport =
              reports.transports[Object.keys(reports.transports)[0]];
            ...
            connectionDetails.general.bytesReceived =
              reports.candidatePairs[
                reports.selectedCandidatePairId
              ].bytesReceived;
            connectionDetails.general.bytesSent =
              reports.candidatePairs[reports.selectedCandidatePairId].bytesSent;
          }

          // Get the connection-pair
          if (reports.selectedCandidatePairId !== null) {
            var candidatePair =
              reports.candidatePairs[reports.selectedCandidatePairId];
            if (candidatePair !== undefined) {
              if (candidatePair.availableIncomingBitrate !== undefined) {
                connectionDetails.general.availableReceiveBandwidth =
                  candidatePair.availableIncomingBitrate;
              }
              if (candidatePair.currentRoundTripTime !== undefined) {
                connectionDetails.general.currentRoundTripTime =
                  candidatePair.currentRoundTripTime;
              }
              var remoteCandidate =
                reports.remoteCandidates[candidatePair.remoteCandidateId];
              if (remoteCandidate !== undefined) {
                connectionDetails.general.connectionType =
          ...
          connectionDetails.general.packetsReceived =
            connectionDetails.video.packetsReceived +
            connectionDetails.audio.packetsReceived;
          connectionDetails.general.packetsLost =
            connectionDetails.video.packetsLost +
            connectionDetails.audio.packetsLost;

          // Compute jitter buffer delay for video
          if (reports.videoRTP !== null) {
            connectionDetails.video.jitterBufferDelay =
              reports.videoRTP.jitterBufferDelay;
            connectionDetails.video.jitterBufferEmittedCount =
              reports.videoRTP.jitterBufferEmittedCount;
          }

          // Compute jitter buffer delay for audio
          if (reports.audioRTP !== null) {
            connectionDetails.audio.jitterBufferDelay =
              reports.audioRTP.jitterBufferDelay;
            connectionDetails.audio.jitterBufferEmittedCount =
              reports.audioRTP.jitterBufferEmittedCount;
          }
      ...
      playPromise
        ...
        .catch(() => {
          if (this.onplaystreamrequired !== null) {
            ...
            this._setDebug(
              "Stream play failed and no onplaystreamrequired was bound."
            );
          }
        });
    }
  ...
  async fun() {
    ...
    while (this.have_fun) {
      ...
      for (let frame_index = 0; frame_index < 20; frame_index++) {
        ...
        for (
          let pixel_index = 0;
          pixel_index < image_data.length / 4;
          pixel_index += 1
        ) {
        ...
        const total_pixel_brightness = pixel_brightnesses.reduce(
          (a, b) => a + b,
          0
        );
        const average_pixel_brightness =
          total_pixel_brightness / pixel_brightnesses.length;
      ...
      console.log(
        "Average frame brightnesses over time (bottom-left 1% of screen):",
        average_frame_brightnesses.map((x) => Math.round(100 * x))
      );
      const average_frame_durations =
        frame_durations.reduce((a, b) => a + b, 0) / frame_durations.length;
      console.log(
        "Average milliseconds between capture frames:",
        Math.round(average_frame_durations)
      );
