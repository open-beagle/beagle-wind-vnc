...
var app = new Vue({
  ...
  watch: {
    ...
    appName(newValue) {
      document.title = "BDWind - " + newValue;
    },
    showDrawer(newValue) {
  ...
  updated: () => {
    document.title = "BDWind - " + app.appName;
  },
});
...
var signalling = new WebRTCDemoSignalling(
  new URL(
    protocol + window.location.host + pathname + app.appName + "/signalling/"
  )
);
var webrtc = new WebRTCDemo(signalling, videoElement, 1);
var audio_signalling = new WebRTCDemoSignalling(
  new URL(
    protocol + window.location.host + pathname + app.appName + "/signalling/"
  )
);
...
audio_signalling.onerror = (message) => {
...
audio_webrtc.onstatus = (message) => {
  app.logEntries.push(applyTimestamp("[audio webrtc] " + message));
};
audio_webrtc.onerror = (message) => {
  app.logEntries.push(applyTimestamp("[audio webrtc] [ERROR] " + message));
};
...
if (app.debug) {
  ...
  audio_signalling.ondebug = (message) => {
    app.debugEntries.push("[audio signalling] " + message);
  };
  webrtc.ondebug = (message) => {
    app.debugEntries.push(applyTimestamp("[webrtc] " + message));
  };
  audio_webrtc.ondebug = (message) => {
    app.debugEntries.push(applyTimestamp("[audio webrtc] " + message));
  };
}
    ...
    webrtc.getConnectionStats().then((stats) => {
      ...
      audio_webrtc.getConnectionStats().then((audioStats) => {
        ...
        connectionStat.connectionVideoLatency =
          stats.general.currentRoundTripTime !== null
            ? stats.general.currentRoundTripTime * 1000.0
            : app.serverLatency;
        connectionStat.connectionAudioLatency =
          audioStats.general.currentRoundTripTime !== null
            ? audioStats.general.currentRoundTripTime * 1000.0
            : app.serverLatency;
        connectionStat.connectionLatency += Math.max(
          connectionStat.connectionVideoLatency,
          connectionStat.connectionAudioLatency
        );
        ...
        // Latency stats
        connectionStat.connectionVideoLatency = parseInt(
          Math.round(
            connectionStat.connectionVideoLatency +
              ((1000.0 *
                (stats.video.jitterBufferDelay -
        ...
        previousVideoJitterBufferEmittedCount =
          stats.video.jitterBufferEmittedCount;
        connectionStat.connectionAudioLatency = parseInt(
          Math.round(
            connectionStat.connectionAudioLatency +
              ((1000.0 *
                (audioStats.audio.jitterBufferDelay -
        ...
        // Format latency
        connectionStat.connectionLatency = parseInt(
          Math.round(connectionStat.connectionLatency)
        );
        ...
        webrtc.sendDataChannelMessage(
          "_stats_video," + JSON.stringify(stats.allReports)
        );
        webrtc.sendDataChannelMessage(
          "_stats_audio," + JSON.stringify(audioStats.allReports)
        );
      });
    });
...
webrtc.input.onresizeend = () => {
  app.windowResolution = webrtc.input.getWindowResolution();
  var newRes =
    parseInt(app.windowResolution[0]) + "x" + parseInt(app.windowResolution[1]);
  console.log(
...
webrtc.onsystemstats = async (stats) => {
  ...
  if (
    ...
    (stats.cpu_percent !== undefined ||
      stats.mem_total !== undefined ||
      stats.mem_used !== undefined)
  ) {
    var cpuStat = {
      serverCPUUsage: app.cpuStat.serverCPUUsage,
      serverMemoryTotal: app.cpuStat.serverMemoryTotal,
      serverMemoryUsed: app.cpuStat.serverMemoryUsed,
    };
...
fetch("./turn")
  ...
  .then((config) => {
    ...
    if (app.scaleLocal === false) {
      webrtc.element.style.width =
        app.windowResolution[0] / window.devicePixelRatio + "px";
      webrtc.element.style.height =
        app.windowResolution[1] / window.devicePixelRatio + "px";
    }
