...
# 启动Steam游戏
# LUTRIS_APP_ID环境变量由用户指定要运行的游戏ID
if [ -n "${LUTRIS_APP_ID}" ]; then
  echo "Starting Steam game with AppID: ${LUTRIS_APP_ID}"
  
  # 启动游戏
  /usr/games/lutris lutris:rungameid/${LUTRIS_APP_ID} &
  ...
  while [ "$WAIT_TIME" -lt "$MAX_WAIT_TIME" ]; do
    ...
    GAME_PID=$(ps -ef | grep -E "rungameid/${LUTRIS_APP_ID}" | grep -v grep | awk '{print $2}')
  ...
  echo "No LUTRIS_APP_ID specified, continuing without launching Steam"
fi
